...
**Проверено в поле:** SRTLA с бондингом Wi-Fi и соты, RTMP (в том числе прямо
на Twitch), адаптивный битрейт, сцены, чат, донат-оверлеи, переподключение при
потере канала.
...
- Для бондинга — несколько сетевых интерфейсов одновременно (обычно Wi-Fi
  и мобильные данные).
- Приёмная сторона, понимающая SRTLA: например
  [bbox-receiver](https://github.com/datagutt/bbox-receiver) на своём сервере.
  Для обычного RTMP свой сервер не нужен — подойдёт ингест площадки.
